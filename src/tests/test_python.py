...
class TestPythonPlugin(NmkBaseTester):
    ...
    def test_python_version(self, monkeypatch: pytest.MonkeyPatch):
    ...
    def fake_python_src(self, content: str = "", package: str = "fake", name: str = "fake.py", with_init: bool = False):
    ...
    def test_python_editable_skipped(self, monkeypatch: pytest.MonkeyPatch):
...
""",
    ...
    def test_install_ok(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(VenvUpdateBuilder, "build", lambda _s, pip_args: None)  # type: ignore

        # Prepare test project for python install
        self.fake_python_src("")
        self.nmk(self.prepare_project("ref_python.yml"), extra_args=["py.install"])

    def test_install_skipped(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr("nmk_python.build.is_windows", lambda: True)
    ...
    def test_current_version_not_compatible(self, monkeypatch: pytest.MonkeyPatch):
        # Fake python version
