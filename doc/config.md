...
(pythonIgnoredLockfile)=

### **`pythonIgnoredLockfile`** -- Ignored lock files

| Type      | Default value                                                           |
| --------- | ----------------------------------------------------------------------- |
| list[str] | Generated by {py:class}`nmk_python.build.PythonIgnoredLockfileResolver` |

Depending on the used backend, resolves to a list of lock files to be git-ignored (i.e. contributed to [`${gitIgnoredFiles}`](https://nmk-base.readthedocs.io/en/stable/config.html#gitignoredfiles-git-ignored-files-list)):

- if **uv** backend is used and is not locked: ignore the **uv.lock** file
- otherwise, no files to ignore

_<span style="color:green">Added in version 1.10.0</span>_
...
This is the current project version (respecting [PyPa version specifiers](https://packaging.python.org/en/latest/specifications/version-specifiers/#version-specifiers)), dynamically computed thanks to [setuptools_scm](https://setuptools-scm.readthedocs.io/en/latest/)

Computing is controlled thanks to {ref}`${pythonVersionScheme}<pythonVersionScheme>`, {ref}`${pythonVersionLocalScheme}<pythonVersionLocalScheme>` and {ref}`${pythonVersionFallback}<pythonVersionFallback>` config items

```{note}
*<span style="color:orange">Behavior changed in version 1.11.0</span>*<br>
In former versions, this version was derived from [${gitVersion}](https://nmk-base.readthedocs.io/en/stable/config.html#gitversion-git-version).
```
...
### **`pythonVersionScheme`** -- Used version scheme for python version

| Type | Default value  |
| ---- | -------------- |
| str  | "no-guess-dev" |

Used version scheme for [setuptools_scm](https://setuptools-scm.readthedocs.io/en/latest/config/)

_<span style="color:green">Added in version 1.11.0</span>_

---

(pythonVersionLocalScheme)=

### **`pythonVersionLocalScheme`** -- Used local scheme for python version

| Type | Default value   |
| ---- | --------------- |
| str  | "node-and-date" |

Used local scheme for [setuptools_scm](https://setuptools-scm.readthedocs.io/en/latest/config/)
...
### **`pythonVersionFallback`** -- Fallback python version

| Type | Default value |
| ---- | ------------- |
| str  | "0.0.0"       |

Fallback version for [setuptools_scm](https://setuptools-scm.readthedocs.io/en/latest/config/) (if version computing from scm fails)
...
### **`pythonConstraints`** -- List of python dependencies constraints

| Type      | Default value |
| --------- | ------------- |
| list[str] | []            |

List of python dependencies constraints statements.

_<span style="color:green">Added in version 1.11.0</span>_

---

(pythonConstraintsFiles)=

### **`pythonConstraintsFiles`** -- List of python dependencies constraints files

| Type      | Default value |
| --------- | ------------- |
| list[str] | []            |

List of python dependencies constraints files.
...
### **`pythonMergedConstraints`** -- Merged list of python dependencies

| Type      | Default value                                                             |
| --------- | ------------------------------------------------------------------------- |
| list[str] | Generated by {py:class}`nmk_python.build.PythonMergedConstraintsResolver` |

List of applyable python dependencies constraints, merged from {ref}`${pythonConstraints}<pythonConstraints>` and {ref}`${pythonConstraintsFiles}<pythonConstraintsFiles>` content.

If using **uv** env backend, this list will be added to the [tool.uv.constraint-dependencies](https://docs.astral.sh/uv/reference/settings/#constraint-dependencies) section of the **pyproject.toml** file.
